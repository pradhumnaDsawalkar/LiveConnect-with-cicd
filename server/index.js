...
io.on('connection',(socket)=>{
  ...
  socket.on('join',async ({name,room,language},callback)=>{   //This is an event listener of emitted event from client side 
  ...
  roomUsers.forEach(async (u) => {
      const translatedText = await translateMessage(`${user.name} has joined!`, u.language || "en");
  ...
  if (!message || typeof message !== "string") {
    return callback({ error: "Invalid message format" });
      ...
      usersInRoom.forEach(async (u) => {
        const translatedText = await translateMessage(message, u.language || "en");
